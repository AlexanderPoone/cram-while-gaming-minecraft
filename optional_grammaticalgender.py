...
Add indefinite article with grammatical gender in front of each item/block name for learners of languages with gender. The gender is obtained from re-structured Wiktionary dumps.
...
words= [trans[k].split(' ')[0].lower() for k in trans if k.startswith('item.minecraft') or k.startswith('block.minecraft') or k.startswith('entity.minecraft')]
...
for mod in MOD_LIST:
	try:
		...
		new = sorted(set([mod_trans[k].split(' ')[0].lower() for k in mod_trans if len(mod_trans[k].split(' ')[0]) > 2 and (k.startswith('item.') or k.startswith('block.') or k.startswith('entity.'))]))
		print(new)
		words += new
...
for word in words:
	...
	try:
		word = word.lower()
		url = f'https://kaikki.org/dictionary/French/meaning/{word[0]}/{word[:2]}/{word}.json'
		print(url)
		res=get(url)
		tags = loads([x for x in res.text.split('\n') if x.startswith('{"pos": "noun')][0])['senses'][0]['tags']
		out_display = 'p' if 'plural' in tags else ('m' if 'masculine' in tags else ('f' if 'feminine' in tags else None))
